...
void print_usage(const char* prog_name) {
    printf("用法: %s [线程数] [子表数量] [插入次数]\n", prog_name);
    printf("  线程数: 插入数据的线程数\n");
    printf("  子表数量: 要创建的子表数量\n");
    printf("  插入次数: 每个子表插入的记录数\n");
    printf("示例: %s 16 10000 1000 - 使用16个线程，自动创建1万个子表并每个子表写入1000条记录\n", prog_name);
}
...
void insert_data_thread(TAOS* taos, int start_id, int end_id, int rows_per_table, int thread_id) {
    ...
    for (int table_id = start_id; table_id < end_id; table_id++) {
        ...
        // 准备SQL语句 - 使用超级表名
        std::string sql = "INSERT INTO ? USING history TAGS(?, ?) VALUES(?, ?, ?, ?, ?)";
        ...
        // 准备TAG数据
        std::string tag_i = "SN_" + std::to_string(table_id);
        int8_t tag_c = c_dist(gen);

        // 绑定TAG参数
        TAOS_MULTI_BIND tags[2];

        // TAG i (VARCHAR)
        int32_t tag_i_len = tag_i.length();
        tags[0].buffer_type = TSDB_DATA_TYPE_VARCHAR;
        tags[0].buffer_length = tag_i.length();
        tags[0].buffer = (void*)tag_i.c_str();
        tags[0].length = &tag_i_len;
        tags[0].is_null = NULL;
        tags[0].num = 1;

        // TAG c (TINYINT)
        int32_t tag_c_len = sizeof(int8_t);
        tags[1].buffer_type = TSDB_DATA_TYPE_TINYINT;
        tags[1].buffer_length = sizeof(int8_t);
        tags[1].buffer = &tag_c;
        tags[1].length = &tag_c_len;
        tags[1].is_null = NULL;
        tags[1].num = 1;

        // 设置表名和TAG，自动创建子表
        if (taos_stmt_set_tbname_tags(stmt, table_name.c_str(), tags) != 0) {
            log_error("线程%d为表%s设置表名和TAG失败: %s", thread_id, table_name.c_str(), taos_stmt_errstr(stmt));
            taos_stmt_close(stmt);
            continue;
        }
        ...
        if (table_id % 10 == 0 || table_id == end_id - 1) {
            ...
            total_tables_created += local_tables_created;
            local_inserted = 0;
...
int main(int argc, char** argv) {
    ...
    table_count = atoi(argv[2]);
    rows_per_table = atoi(argv[3]);
    ...
    log_info("准备测试: 线程数=%d, 子表数量=%d, 每表行数=%d",
             thread_count, table_count, rows_per_table);
    ...
    log_info("子表数量: %d", total_tables_created.load());
    ...
    log_info("插入数据速率: %.2f 万行/秒", total_rows_inserted.load() / total_elapsed.count() / 10000);

...
void print_usage(const char* prog_name) {
    printf("用法: %s [测点数量N] [重复写入次数M] [每批处理表数X]\n", prog_name);
    printf("  测点数量N: 将创建N个子表，默认为1000000\n");
    printf("  重复写入次数M: 每个测点重复写入M次，默认为1\n");
    printf("  每批处理表数X: 每批处理X个表，默认为50\n");
    printf("示例: %s 100000 10 50 - 创建10万个测点，每个测点写入10次，每批处理50个表\n", prog_name);
}
...
int main(int argc, char** argv) {
    ...
    int tables_per_batch = 50;   // 每批处理的表数量，默认值改为50，避免内存溢出
    int records_per_table = 10;  // 每个表每批次的记录数限制，防止SQL过大
    ...
    if (argc > 3) {
        tables_per_batch = atoi(argv[3]);
        if (tables_per_batch <= 0) {
            log_error("无效的每批处理表数: %s", argv[3]);
            print_usage(argv[0]);
            return 1;
        }
    }
    ...
    // 限制每批处理的表数，防止内存溢出
    if (tables_per_batch > 200) {
        log_warning("每批处理表数过大，已自动调整为200，以防止内存溢出");
        tables_per_batch = 200;
    }

    int total_records = points_count * repeat_times;
    log_info("准备写入测试: 测点数量=%d, 重复次数=%d, 每批处理表数=%d, 总记录数=%d",
             points_count, repeat_times, tables_per_batch, total_records);
    ...
    // 创建数据库
    TAOS_RES* res = taos_query(taos, "CREATE DATABASE IF NOT EXISTS test4");
    ...
    for (int batch = 0; batch < total_batch_count; batch++) {
        ...
        for (int i = start_idx; i < end_idx; i++) {
            ...
            const std::vector<InsertData> &all_records = table_data_map[table_id];
            ...
            // 限制每个表每批次的记录数，防止SQL过大
            int record_count = std::min((int)all_records.size(), records_per_table);

            bool first_record = true;
            for (int j = 0; j < record_count; j++) {
                const InsertData &data = all_records[j];
        ...
        while (!success && retry_count < max_retries) {
            ...
            } else if (code == 0x2600) { // 内存限制错误
                taos_free_result(res);
                log_warning("批次%d内存限制错误，等待1秒后重试 (尝试 %d/%d)",
                            batch, retry_count + 1, max_retries);
        ...
        if (!success) {
            log_error("批次%d在%d次尝试后仍然失败，跳过", batch, max_retries);
        }
        ...
        if (batch % (total_batch_count / 10 + 1) == 0 || batch == total_batch_count - 1) {
            ...
            log_info("写入进度: %.1f%%，已处理 %d/%d 批次，已写入 %d 条记录，失败 %d 批次，耗时 %.2f 秒，写入速率 %.2f 条/秒",
                     progress, batch + 1, total_batch_count, total_written,
                     failed_batches, elapsed.count(), write_rate);
        }
    }
    ...
    log_info("每表每批写入记录数上限: %d", records_per_table);
    log_info("总批次数: %d", total_batches);
    log_info("失败批次数: %d", failed_batches);
